...
class AnalysisEngine:
    ...
    def generate_analysis(self, topic: str, methods: tuple[str, str]) -> str:
        ...
        analysis: str = "\n\n".join(sections)
        if text_length(analysis) < 1500:
            extensions: list[str] = [
                "추가적으로, 실행 단계마다 '관찰-해석-개선' 주기를 명확히 정의해야 하며 정기 리뷰에서는 성과뿐 아니라 미해결 가정과 잠재 리스크를 함께 점검해야 한다.",
                "또한 이해관계자 간 해석 차이를 줄이기 위해 공통 용어집을 운영하고, 분기별로 우선순위를 재설정하는 전략 리셋 미팅을 제도화해야 한다.",
                "사용자 피드백을 단순 만족도 조사에 그치지 않고 행동 변화 데이터와 연결해 해석함으로써 실제 효과를 더 정밀하게 파악할 수 있다.",
                "조직의 문화적 관점에서도 이러한 시스템 사고의 도입은 구성원들의 문제 해결 능력을 상향 평준화하는 효과를 가져온다.",
                "단순히 도구를 제공하는 것을 넘어, 사고의 프레임을 공유함으로써 부서 간의 사일로 현상을 완화하고 협업의 품질을 근본적으로 개선할 수 있다.",
                "장기적인 관점에서는 이러한 데이터 기반의 통찰이 축적되어 조직의 고유한 지식 자산이 되며, 이는 시장 변화에 대한 민첩한 대응력으로 이어진다.",
                "결국 시스템 사고의 핵심은 고정된 계획이 아니라 변화하는 환경 속에서 끊임없이 가설을 검증하고 방향을 수정해 나가는 동적 역량에 있다.",
                "따라서 초기 도입 시에는 완벽한 시스템을 구축하려 하기보다, 작은 성공 사례를 빠르게 만들어 조직 내 확산 동력을 확보하는 것이 무엇보다 중요하다.",
                "이 과정에서 리더십은 단기적인 성과 지표에 일희일비하지 않고, 학습과 개선이라는 시스템의 본질적 가치에 힘을 실어주어야 한다.",
                "지속 가능한 혁신은 한 번의 천재적인 아이디어가 아니라, 수많은 작은 실험과 그로부터 얻은 교훈이 쌓여 만들어지는 견고한 시스템의 산물이다."
            ]
        ...
        while text_length(analysis) < 1500:
            analysis += (
